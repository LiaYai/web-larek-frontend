...
Данные заказа в первом модальном окне
...
- `preview` - id карточки товара, который выбрали для препросмотра.
- `basket` - массив из id товаров, которые помещены в корзину.
...
Сеттеры и геттеры:
- `id` - установить/получить id карточки.
- `title` - установить/получить заголовок карточки.
- `category` - установить/получить категорию товара.
- `image` - установить изображения товара.
- `description` - установить описание товара.
- `price` - установить/получить цену товара.
...
- `totalElement` - элемент содержащий общую стоимость заказа в `textContent`.
- `button` - элемент кнопки.

Конструктор принимает в качестве аргументов селектор, по которому в разметке будет выбран темплейт окна Success, стоимость заказа и экземпляр класса EventEmitter. В конструкторе каждому полю присваивается соответствующий элемент разметки. Кнопке навешивается слушатель, который вызывает событие сабмита. 

Имеет сеттер, который устанавливает в поле `totalElement` сумму `total`.
...
- `button` - элемент кнопки сабмита внутри корзины.
...
- `selected` - сеттер для кнопки сабмита, делает ее активной, если список товаров не пуст и наоборот.
...
`catalog:changed` - происходит при загрузке страницы. Добавляет товары в каталог и добавляет число товаров в корзине на изображение корзины.

`card:select` - происходит при нажатии на карточку товара в каталоге. Присваивает preview id товара и вызывает `preview:changed`.

`preview:changed` - открывает модальное окно и отрисовывает в нем карточку товара.

`basket:open` - отрисовывает разметку корзины.

`basket:changed` - происходит при добавлении или удалении товара из корзины, завершении заказа. Вызывает изменение разметки корзины, обновление счетчика корзины на главной странице, очистку корзины.

`orderInfo:open` - происходит при клике на кнопку "Оформить" в корзине. Вызывает отрисовку первой формы заказа.

`orderContacts:open` - происходит при клике на кнопку "Далее" в первом окне заказа. Вызывает отрисовку второй формы заказа.

`order:submit` - происходит при клике на кнопку "Оплатить" во втром окне заказа. Вызывает отправление данных заказа, очистку корзины, данных заказа и открытие окна подтверждения заказа.
