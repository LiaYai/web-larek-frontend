...
`card:select` - происходит при нажатии на карточку товара в каталоге. Вызывает событие `preview:changed`.

`preview:changed` - открывает модальное окно и отрисовывает в нем карточку товара.

`basket:open` - отрисовывает разметку корзины.

`basketItems:changed` - происходит при добавлении или удалении товара в корзине, в окне превью товара. Вызывает событие `basket:changed`.
